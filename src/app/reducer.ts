...
// import CanisGenerator, { ICanisSpec, IAnimationSpec } from './core/canisGenerator'
...
// import { IKeyframeGroup, IActivatePlusBtn, ISize, ICoord, IPath, IAttrAndSpec } from '../redux/global-interfaces'
// import { store } from '../redux/store'
// import {  } from '../redux/action/videoAction'
// import { updateSelection } from '../redux/action/chartAction'
// import { resetSpec } from '../redux/action/canisAction'
...
export default class Reducer {
  ...
  public static saveAndTriger(
    ...
    //     historyAction: { actionType: action, actionVal: oldActionVal },
    //     currentAction: { actionType: action, actionVal: newActionVal }
    // })
    // State.saveHistory();
    // this.triger(action, newActionVal);
  }

  // public static updateAniAlign(actionType: string, actionInfo: { targetAniId: string, currentAniId: string }) {
  //     const animations: IAnimationSpec[] = store.getState().spec.animations;
...
// })
// Reducer.listen(action.UPDATE_SUGGESTED_MARKS, (sm: string[]) => {
//     state.suggestedMarks = sm;
// })
// Reducer.listen(action.UPDATE_MARKS_TO_CONFIRM, (mc: string[][]) => {
//     state.marksToConfirm = mc;
// })
...
// })
// Reducer.listen(action.UPDATE_KEYFRAME_CONTAINER_SLIDER, (kfGroupSize: ISize) => {
...
// Reducer.listen(action.UPDATE_IS_SKETCHING, (actionInfo: { sketching: boolean, coord: ICoord }) => {
//     state.sketching = actionInfo.sketching;
//     if (state.sketching && !state.isPreviewing) {
//         // Reducer.triger(action.UPDATE_ISPLAYING, false);
//         store.dispatch(updatePlaying(false));
//     }
//     Renderer.renderSketchCanvas(actionInfo.coord);
// })

// Reducer.listen(action.UPDATE_ISPLAYING, (p: boolean) => {
//     state.isPlaying = p;
// })

// Reducer.listen(action.UPDATE_TOUCH_TYPE, (touchType: number) => {
//     state.touchType = touchType;
// })
...
// Reducer.listen(action.UPDATE_EFFECT_AND_DURATION, (actionInfo: { aniId: string, effect: string, duration: number }) => {
//     const animations: IAnimationSpec[] = state.spec.animations;
...
//             a.effects = [CanisGenerator.initEffect(actionInfo.effect, actionInfo.duration)];
//         }
//     })
//     state.spec = { ...state.spec, animations: animations };
// })
...
//         // })
//         // state.spec = { ...state.spec, animations: animations };
//         state.spec = StateModifier.updateGrouping(actionInfo.aniId, actionInfo.attrComb, actionInfo.attrValueSort);
//     }
// })
...
//     // }
//     // state.spec = { ...state.spec, animations: animations };
//     state.spec = StateModifier.removeAndCreateMulti(actionInfo.aniId, actionInfo.path, actionInfo.attrValueSort);
// })
...
//     // }
//     // state.spec = { ...state.spec, animations: animations };
//     state.spec = StateModifier.splitCreateMultiAni(actionInfo.aniId, actionInfo.path, actionInfo.attrValueSort);
// })
...
//     // }
//     // state.spec = { ...state.spec, animations: animations };
//     state.spec = StateModifier.splitCreateOneAni(actionInfo.aniId, actionInfo.newAniSelector, actionInfo.attrComb, actionInfo.attrValueSort);
// })
...
//     // }
//     // state.spec = { ...state.spec, animations: animations };
//     state.spec = StateModifier.appendGrouping(actionInfo.aniId, actionInfo.attrComb, actionInfo.attrValueSort);
// })
...
// Reducer.listen(action.UPDATE_SELECTED_MARKS, (mIds: string[]) => {
//     mIds.forEach((mId: string) => {
//         const className: string = Animation.markClass.get(mId);
//         if (typeof className !== 'undefined') {
//             if (typeof state.selectedMarks.get(className) === 'undefined') {
//                 state.selectedMarks.set(className, []);
//             }
//             const tmpSet: Set<string> = new Set(state.selectedMarks.get(className));//remove redundant
//             tmpSet.add(mId);
//             state.selectedMarks.set(className, [...tmpSet]);
//         }
//     })
//     Renderer.renderSelectedMarks();
// })

// Reducer.listen(action.TOGGLE_SYS_TOUCH, (st: boolean) => {
//     state.systemTouch = st;
// })

// Reducer.listen(action.UPDATE_ACTIVE_SLIDER, (slider: Slider) => {
//     state.activeSlider = slider;
// })

// Reducer.listen(action.UPDATE_SUGGEST_SPECS, (specs: IAttrAndSpec[]) => {
//     state.suggestSpecs = specs;
// })

// Reducer.listen(action.UPDATE_PREVIEWING, (actionInfo: { previewPath: IPath, previewSpec: ICanisSpec, preview: boolean, switchSpec: boolean }) => {
//     if (state.isPreviewing !== actionInfo.preview) {
//         if (actionInfo.switchSpec) {
//             state.isPreviewing = actionInfo.preview;
//             state.previewPath = actionInfo.previewPath;
//             state.previewSpec = actionInfo.previewSpec;
//             Renderer.renderSuggestSpec(actionInfo.preview ? actionInfo.previewSpec : state.spec, actionInfo.preview, actionInfo.preview);
//         }
//         // (!actionInfo.preview) && Reducer.triger(action.UPDATE_ISPLAYING, false);
//         (!actionInfo.preview) && store.dispatch(updatePlaying(false));
//     }
// })

// Reducer.listen(action.VOICE_AWAKE, (va: boolean) => {
//     state.voiceAwake = va;
// })

// Reducer.listen(action.UPDATE_SELECTION_ORDER, (so: IOrderInfo) => {
//     state.selectionOrder = so;
// })

// Reducer.listen(action.UPDATE_HIGHLIGHT_KF, (kf: KfItem) => {
//     state.highlightKf = kf;
// })

// Reducer.listen(action.MARK_IN_NEW_CREATE_KFG, (marks: string[]) => {
//     state.marksInNewCreateKFG = marks;
// })
