...
export default class CanisGenerator {
  public static generateChartSpec(charts: string[]): IChartSpec[] {
    ...
    for (let i = 0; i < charts.length; i++) {
      ...
      const chartType: string =
        charts[i].indexOf("<svg") >= 0
          ? ChartSpec.CHART_CONTENT
          : ChartSpec.CHART_URL;
  ...
  public static validate(spec: ICanisSpec): boolean {
        ...
        if (typeof tmpAni.id !== "undefined") {
          ...
          removedAni.sort((a: IAnimationSpec, b: IAnimationSpec) => {
            if (
              ...
              b.reference === TimingSpec.timingRef.previousStart
            ) {
              return 1;
            } else if (
              a.reference === TimingSpec.timingRef.previousStart &&
              b.reference === TimingSpec.timingRef.previousEnd
            ) {
              return -1;
            }
            return 0;
        ...
        effects: [
          {
            type:
              spec.charts.length === 1
                ? ActionSpec.actionTypes.fade
                : ActionSpec.actionTypes.transition,
            duration: 300,
          },
        ],
      };
    ...
    if (spec.charts.length > 1) {
      ...
      for (let i = 0, len = spec.animations.length; i < len; i++) {
        ...
        for (
          let j = 0, len2 = spec.animations[i].effects.length;
          j < len2;
          j++
        ) {
  ...
  public static updateKfRefAndDelay(
    groupingSpec: IGrouping,
    ref: string,
    delay: number
  ): void {
    if (typeof groupingSpec.grouping !== "undefined") {
  ...
  public static updateGroupTiming(
    groupingSpec: IGrouping,
    groupRef: string,
    ref: string
  ): void {
  ...
  public static updateGroupDelay(
    groupingSpec: IGrouping,
    groupRef: string,
    delay: number
  ): void {
  ...
  public static updateGroupDelayTiming(
    groupingSpec: IGrouping,
    groupRef: string,
    delay: number,
    ref: string
  ...
  public static removeGroupDelay(
    groupingSpec: IGrouping,
    groupRef: string
  ): void {
    if (groupingSpec.groupBy === groupRef) {
  ...
  public static removeGroupDelayUpdateTiming(
    groupingSpec: IGrouping,
    groupRef: string,
    ref: string
  ): void {
  ...
  public static mergeGroup(
    ...
    if (groupingSpec.groupBy === groupRef) {
      ...
      if (typeof groupingSpec.grouping !== "undefined") {
        groupingSpec.grouping = this.mergeGroup(
          groupingSpec.grouping,
          groupRef,
          groupingSpec
        );
  ...
  public static initEffect(effect?: string, duration?: number): IAction {
    return {
      type:
        typeof effect === "undefined" ? ActionSpec.actionTypes.fade : effect,
  ...
  public static updateStaticSelector(
    ani: IAnimationSpec,
    staticMarks: string[]
  ): void {
  ...
  public static updateGrouping(
    parent: IAnimationSpec | IGrouping,
    attrComb: string[],
  ...
  public static createGrouping(
    parent: IAnimationSpec | IGrouping,
    attrComb: string[],
    attrValueSort?: string[][]
  ) {
    //copy
    let attrCombNew = JSON.parse(JSON.stringify(attrComb));
    let attrValueSortNew = JSON.parse(JSON.stringify(attrValueSort));

    parent.grouping = {
      groupBy: attrCombNew[0],
      reference: TimingSpec.timingRef.previousEnd,
    };
    if (typeof attrValueSortNew !== "undefined") {
      parent.grouping.sort = {
        order: attrValueSortNew[0],
      };
      attrValueSortNew.splice(0, 1);
    }
    attrCombNew.splice(0, 1);
    if (attrCombNew.length > 0) {
      this.createGrouping(parent.grouping, attrCombNew, attrValueSortNew);
    }
  }

  public static appendGrouping(
    parent: IAnimationSpec | IGrouping,
    attrComb: string[],
    attrValueSort?: string[][]
  ) {
  ...
  public static removeMarksFromSelector(
    ani: IAnimationSpec,
    targetMarks: string[]
  ) {
  ...
  public static findMergedAlignAnis(id: string): [string, string[]] {
    let alignWithAni: string,
      alignToAnis: string[] = [];
    store.getState().spec.animations.forEach((a: IAnimationSpec) => {
      if (a.id === id) {
  ...
  public static updateGroupingSort(
    a: IAnimationSpec,
    groupRef: string,
    order: string[]
  ): boolean {
