...
export default class Artyom {
   ...
   */
  private ArtyomVoicesIdentifiers: any;
  ...
  addCommands(param: any) {
    ...
    let processCommand = (command: any) => {
  ...
  debug(message: string, type?: string) {
    ...
    // console.log('test!!!!!', this.ArtyomProperties.debug, type, message);
    if (this.ArtyomProperties.debug === true) {
      ...
      switch (type) {
        ...
        default:
          ...
          const voiceStr: string = message.split(":")[1] || "";
          // console.log(voiceStr, store.getState().voiceAwake);
          if (store.getState().voiceAwake) {
            const voiceContainer: HTMLElement =
              document.getElementById("recogVoice");
            if (typeof voiceContainer !== "undefined") {
              const voiceStr: string = message.split(":")[1] || "";
              let isVoice: boolean = true;
              Object.keys(this.ArtyomGlobalEvents).forEach(
                (evtName: string) => {
                  if (this.ArtyomGlobalEvents[evtName] === voiceStr.trim()) {
                    isVoice = false;
                  }
                }
              );
              if (isVoice) {
                voiceContainer.setAttribute("value", voiceStr);
              }
            }
          }
          // console.log(
  ...
  execute(voz: any): any {
    ...
    if (_this.ArtyomProperties.soundex) {
      for (let i = 0; i < _this.ArtyomCommands.length; i++) {
        let instruction: any = _this.ArtyomCommands[i];
        let opciones = instruction.indexes;
        let encontrado = -1;
  ...
  hey(resolve: Function, reject: Function) {
     ...
     * @returns {undefined}
     */
    this.ArtyomWebkitSpeechRecognition.onerror = (event: any) => {
      ...
      onResultProcessor = (event: any) => {
        ...
        for (let i = event.resultIndex; i < cantidadResultados; ++i) {
          ...
          if (event.results[i].isFinal) {
            let comando: any = _this.execute(identificated.trim());
     ...
     * @returns {undefined}
     */
    _this.ArtyomWebkitSpeechRecognition.onresult = (event: any) => {
  ...
  getVoice(languageCode: string) {
    let voiceIdentifiersArray: any = this.ArtyomVoicesIdentifiers[languageCode];

    if (!voiceIdentifiersArray) {
      console.warn(
        `The providen language ${languageCode} isn't available, using English Great britain as default`
      );
  ...
  newDictation(settings: any) {
    ...
    dictado.onresult = function (event: any) {
      let temporal = "";
      let interim = "";
  ...
  newPrompt(config: any) {
    ...
    let promptCommand: any = {
      ...
      action: function (i: any, wildcard: any) {
  ...
  simulateInstruction(sentence: string) {
    ...
    let foundCommand: any = _this.execute(sentence); //Command founded object
  ...
  restart() {
    let _this: Artyom = this;
    let _copyInit: any = _this.ArtyomProperties;
   ...
   * @returns {undefined}
   */
  talk(text: string, actualChunk: number, totalChunks: number, callbacks: any) {
    let _this = this;
  ...
  say(message: string, callbacks?: any) {
    ...
    if (this.speechSupported()) {
      ...
      definitive = definitive.filter((e: any) => e);

      // Finally proceed to talk the chunks and assign the callbacks.
      definitive.forEach((chunk: any, index: any) => {
