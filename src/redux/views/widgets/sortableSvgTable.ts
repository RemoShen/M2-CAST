import "../../assets/style/widgets/sortableSvgTable.scss";
import KfGroup from "../vl/kfGroup";
...
export default class SortableSvgTable {
  ...
  private renderTable() {
    const svgHintLayer: HTMLElement = document.getElementById(KF_HINT_LAYER);
    const layerBBox: DOMRect = document
    ...
    this.container.setAttributeNS(
      ...
      "transform",
      `translate(${
        (this.position.x - layerBBox.left) / store.getState().kfZoomLevel
      }, ${(this.position.y - layerBBox.top) / store.getState().kfZoomLevel})`
    );
    this.container.onmouseleave = () => {
      this.removeTable();
      // this.correspondingKfg.transHideTitle();
    };
    const fakeBg: SVGRectElement = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "rect"
    );
    ...
    const pointer: SVGPathElement = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    pointer.classList.add("drop-shadow-ele");
    pointer.setAttributeNS(null, "d", "M8,0 V12 L0,6 Z");
    pointer.setAttributeNS(
      null,
      "transform",
      `translate(${GROUP_TITLE_HEIHGT - 4}, 2)`
    );
    ...
    bg.setAttributeNS(null, "x", `${GROUP_TITLE_HEIHGT + 4}`);
    bg.setAttributeNS(null, "y", "0");
    bg.setAttributeNS(
      null,
      "width",
      `${SortableSvgTable.TABLE_WIDTH - GROUP_TITLE_HEIHGT - 4}`
    );
    ...
    this.data.forEach((d: string, idx: number) => {
      const itemContainer: SVGGElement = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "g"
      );
      itemContainer.classList.add("ease-transform");
      itemContainer.setAttributeNS(null, "id", `sortAttr${idx}`);
      itemContainer.setAttributeNS(
        null,
        "transform",
        `translate(${
          SortableSvgTable.TABLE_PADDING + GROUP_TITLE_HEIHGT + 4
        }, ${
          SortableSvgTable.TABLE_PADDING + idx * SortableSvgTable.CELL_HEIGHT
        })`
      );
      itemContainer.classList.add("svg-table-cell-container");
      const itemBg: SVGRectElement = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "rect"
      );
      itemBg.setAttributeNS(
        null,
        "width",
        `${
          SortableSvgTable.TABLE_WIDTH -
          2 * SortableSvgTable.TABLE_PADDING -
          GROUP_TITLE_HEIHGT -
          4
        }`
      );
      ...
      const itemContent: SVGTextContentElement = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "text"
      );
      ...
      itemContent.setAttributeNS(
        null,
        "y",
        `${SortableSvgTable.CELL_HEIGHT - 7}`
      );
      ...
      itemContainer.onmousedown = (downEvt) => {
        ...
        let currentItemPosi: ICoord = jsTool.extractTransNums(
          itemContainer.getAttributeNS(null, "transform")
        );
        document.onmousemove = (moveEvt) => {
          ...
          let diffPosi: ICoord = {
            x: 0,
            y: (tmpMousePosi.y - oriMousePosi.y) / store.getState().kfZoomLevel,
          };
          //drag use tag
          const oriTrans: ICoord = jsTool.extractTransNums(
            this.useTag.getAttributeNS(null, "transform")
          );
          this.useTag.setAttributeNS(
            null,
            "transform",
            `translate(${oriTrans.x}, ${oriTrans.y + diffPosi.y})`
          );

          //switch
          if (
            oriTrans.y + diffPosi.y <
            currentItemPosi.y - SortableSvgTable.CELL_HEIGHT / 2
          ) {
            for (let i = 0, len = itemList.length; i < len; i++) {
              const tmpTrans: ICoord = jsTool.extractTransNums(
                itemList[i].getAttributeNS(null, "transform")
              );
              if (
                currentItemPosi.y - SortableSvgTable.CELL_HEIGHT ===
                tmpTrans.y
              ) {
                itemContainer.setAttributeNS(
                  null,
                  "transform",
                  `translate(${currentItemPosi.x}, ${
                    currentItemPosi.y - SortableSvgTable.CELL_HEIGHT
                  })`
                );
                itemList[i].setAttributeNS(
                  null,
                  "transform",
                  `translate(${tmpTrans.x}, ${
                    tmpTrans.y + SortableSvgTable.CELL_HEIGHT
                  })`
                );
        ...
        document.onmouseup = () => {
          ...
          const tmpRecorder: [number, string][] = itemList.map(
            (tmpItemContainer: SVGGElement, idx) => {
              return [
                jsTool.extractTransNums(
                  tmpItemContainer.getAttributeNS(null, "transform")
                ).y,
                itemContentList[idx],
              ];
            }
          ...
          const attrOrder: string[] = tmpRecorder.map(
            (tr: [number, string]) => tr[1]
          );
          this.removeTable();
          const oriOrder: string[] = CanisGenerator.fetchGroupingSort(
            this.correspondingKfg.aniId,
            this.correspondingKfg.childrenRef
          );
