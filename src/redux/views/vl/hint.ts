...
export class Hint {
  ...
  public createHint(mousePosi: ICoord, content: string, w: number = -1): void {
    ...
    const svgHintLayer: HTMLElement = document.getElementById(
      KfContainer.KF_HINT
    );
    ...
    this.container.setAttributeNS(
      ...
      "transform",
      `translate(${
        (mousePosi.x - hintLayerBBox.left) / store.getState().kfZoomLevel + 4
      }, ${
        (mousePosi.y - hintLayerBBox.top) / store.getState().kfZoomLevel + 2
      })`
    );
    this.hintBg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "rect"
    );
    this.hintBg.setAttributeNS(
      null,
      "width",
      `${w === -1 ? Hint.CHAR_LEN * this.content.length : w}`
    );
    this.hintBg.setAttributeNS(
      null,
      "height",
      `${Hint.HINT_HEIGHT + 2 * Hint.PADDING}`
    );
    ...
    this.contentText = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "text"
    );
    ...
    document.onmousemove = (moveEvt) => {
      this.container.setAttributeNS(
        ...
        "transform",
        `translate(${
          (moveEvt.pageX - hintLayerBBox.left) / store.getState().kfZoomLevel +
          4
        }, ${
          (moveEvt.pageY - hintLayerBBox.top) / store.getState().kfZoomLevel + 2
        })`
      );
    };
  }
  ...
  public createTimingHint(
    ...
    this.container.setAttributeNS(
      ...
      "transform",
      `translate(${
        (pointingPosi.x - hintLayerBBox.left) / store.getState().kfZoomLevel -
        hintWidth / 2
      }, ${
        (pointingPosi.y - hintLayerBBox.top) / store.getState().kfZoomLevel -
        Hint.TIMING_HINT_HEIGHT
      })`
    );
    ...
    this.contentText.setAttributeNS(null, "x", `${Hint.PADDING}`);
    this.contentText.setAttributeNS(null, "y", `${Hint.HINT_HEIGHT}`);
    this.contentText.setAttributeNS(
      null,
      "fill",
      `${Hint.TIMING_BORDER_COLOR}`
    );
    this.contentText.innerHTML = `${textBlocks[0]}:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ms`;
    this.container.appendChild(this.contentText);
    this.textWrapper = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "foreignObject"
    );
    this.textWrapper.setAttributeNS(
      null,
      "x",
      `${(textBlocks[0].length + 1) * 8}`
    );
    this.textWrapper.setAttributeNS(
      null,
      "width",
      `${hintWidth - (textBlocks[0].length + 4) * 8}`
    );
    this.textWrapper.setAttributeNS(
      null,
      "height",
    ...
    this.contentInput.classList.add(
      "monospace-font",
      "small-font",
      "hint-input"
    );
    this.contentInput.value = textBlocks[1].substring(
      0,
      textBlocks[1].length - 2
    );
    ...
    this.pointer = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    this.pointer.classList.add("drop-shadow-ele");
    this.pointer.setAttributeNS(
      null,
      "d",
      `M0,0 H6 L3,${
        Hint.TIMING_HINT_HEIGHT - Hint.HINT_HEIGHT - 2 * Hint.PADDING
      } Z`
    );
    this.pointer.setAttributeNS(
      null,
      "transform",
      `translate(${hintWidth / 2 - 6}, ${Hint.HINT_HEIGHT + 2 * Hint.PADDING})`
    );
  ...
  public updateTimingHint(diffX: number, content: string) {
    ...
    const containerTrans: ICoord = Util.extractTransNums(
